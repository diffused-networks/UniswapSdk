    ...
    XunitContextBase(output)
{
    ...
    private static readonly BigInteger POOL_SQRT_RATIO_START = EncodeSqrtRatioX96.Encode(BigInteger.Parse("100e6", NumberStyles.AllowExponent),
        BigInteger.Parse("100e18", NumberStyles.AllowExponent));
    ...
    private static readonly Pool DAI_USDC_POOL = new(DAI, USDC, FeeAmount.LOW, POOL_SQRT_RATIO_START, 0, POOL_TICK_CURRENT, new int[] { });
    ...
    {
        var position = new Position(DAI_USDC_POOL, NearestUsableTick.Find(TickMath.MIN_TICK, TICK_SPACING),
            NearestUsableTick.Find(TickMath.MAX_TICK, TICK_SPACING), 1);
        Assert.Equal(BigInteger.One, position.Liquidity);
    }
